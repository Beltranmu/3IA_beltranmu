...
void Board::rowcol2Index(int row, int col, int* id){
  *id = row * width_ + col;
}
...
int Board::randomWalkableCell()
{
  ...
  while (cell_[rc].value < kTileType_Normal) {
    rc = rand()%(width_ * height_);
  }
  return rc;
}


uint8_t Board::cellType(int id, int cell_type){
  return cell_[id].value == cell_type;
}

bool Board::checkUnitMovement(int unit_id, int id_origin_cell, int id_end_cell)
{
  return true;
}

void Board::moveUnit(int unit_id, int id_origin_cell, int id_end_cell)
{
}
...
void Board::checkAndMove(int unit_id, int id_origin_cell, int id_end_cell){
  ...
  if(cell_[id_end_cell].value >= kTileType_Normal){
    ...
    // Mover graficamente a la unidad
